...
class GTTX {
  ...
  async postDomain(domainName, siteType, vips = [], serviceConfig) {
    ...
    if (serviceConfig) {obj.serviceConfig = serviceConfig;}
    const result = await this._request({
      method,
      uri,
      qs: {
   ...
   * 添加高防域名证书
   * @param {!string} domainName
   * @param {!string} certFile - 公钥
   * @param {!string} certKeyFile - 私钥
   */
  async postDomainCert(domainName, certFile, certKeyFile) {
    const method = 'POST'
    const uri = '/xddos/public/domain/cert'
   ...
   * WEB 基础防护开关
   * @description
   * 开启 WEB 基础防护前允许修改协议端口及源站信息，开启后只允许修改源站 ip；
   * 一条域名记录若存在高防 IP 端口复用的情况，需先开启 WEB 基础防护后，才允许添加
   * 端口复用的域名记录（端口复用的多条记录需同时都开启，才能完全生效，否则，未开
   * 启 WEB 基础防护的记录可能存在访问异常）。端口复用的场景下，WEB 基础防护开启
   * 后不允许关闭，若要修改某条记录，请删除该记录后重新添加。
   * @param {!string} domainName - 域名
   * @param {!string} wafSwitch - "0": 关闭, "1": 开启
   */
  async putDomainBasicWaf(domainName, wafSwitch) {
    const method = 'PUT'
    const uri = '/xddos/public/domain/basicWaf'
   ...
   * 查询回源 IP 段
   * @param {?object} param0
   * @param {!number} param0.type - 回源类型 1 DDoS, 2 WAF
   * @param {?number} param0.line - 线路（1.电信，2.联通，3.移动）
   * @param {?number} param0.location - 地区（1.华东 2.华北 3.华南 4.西北）
   */
  async backSourceIp({type = 1, line, location} = {}) {
    const method = 'GET';
    const uri = '/xddos/public/ipRule/backsourceIp';
    const result = await this._request({
      method,
      uri,
      qs: {
  ...
  async _request(opts, authorize = true) {
    return await retry(async (bail, times) => {
      ...
      const data = await rp(Object.assign(obj, opts))

      debug(`${opts.method} ${opts.uri} respond with data at ${times} times`, obj, data)
